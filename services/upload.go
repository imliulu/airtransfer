...
func UploadFiles(c *gin.Context) {
	...
	for _, file := range files {
		...
		savePath := fmt.Sprintf("uploads/%s/%s", roomID, newFilename)
		...
		fmt.Println("Uploaded file:", uploadedFiles)
	}
...
func ListFiles(c *gin.Context) {
	roomID := c.Param("roomID")
	dirPath := fmt.Sprintf("uploads/%s", roomID)
	...
	for _, entry := range files {
		...
		fileList = append(fileList, gin.H{
			"name":    filename,
			"url":     fmt.Sprintf("/uploads/%s/%s", roomID, filename),
			"size":    info.Size(),
			"isImage": isImageByExt(ext),
		})
	}
